...
@section('content')
<div class="max-w-6xl mx-auto" x-data="{ qrModalOpen: false, activeUuid: '', activeCode: '', cancelModalOpen: false, cancelId: '' }">

    {{-- Header --}}
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
            <h2 class="text-2xl font-extrabold text-gray-800 tracking-tight">Riwayat & Dokumen Izin</h2>
            <p class="text-sm text-gray-500 mt-1">Lihat status pengajuan, tampilkan QR Code, dan cetak dokumen izin Anda.</p>
        </div>
        <a href="{{ route('employee.permit.create') }}" class="bg-gradient-to-r from-mna-dark to-mna-teal text-white px-6 py-3 rounded-xl font-bold shadow-lg shadow-mna-teal/20 hover:shadow-xl hover:-translate-y-0.5 transition-all flex items-center gap-2 text-sm">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        @php
            $allPermits = $permits instanceof \Illuminate\Pagination\LengthAwarePaginator ? $permits->getCollection() : $permits;
            $totalPermits = $allPermits->count();
            $activePermits = $allPermits->whereIn('status', ['approved', 'out'])->count();
            $pendingPermits = $allPermits->where('status', 'pending')->count();
            $completedPermits = $allPermits->where('status', 'returned')->count();
        @endphp
        ...
        <div class="bg-white rounded-2xl p-4 border border-yellow-100 shadow-sm">
            <div class="text-2xl font-extrabold text-yellow-600">{{ $pendingPermits }}</div>
            <div class="text-xs font-bold text-yellow-500 uppercase tracking-wider mt-1">Menunggu</div>
        </div>
        ...
        <div class="flex flex-col lg:flex-row">
            ...
            <div class="flex-1 p-6">
                ...
                @if($permit->status == 'approved')
                    ...
                    @endphp
                    <div class="mt-3 bg-mna-light p-3 rounded-xl border border-mna-teal/10 text-xs text-mna-dark">
                        <i class="fa-solid fa-circle-info text-mna-teal mr-1"></i>
                        Bisa discan mulai jam: <span class="font-extrabold">{{ $earliestAllowedTime->format('H:i') }}</span>
                    </div>
                @endif
                ...
                @if($permit->status == 'returned' && $permit->permit_type == 'pribadi' && $permit->time_in && $permit->target_time_in)
                    ...
                    @if($actualIn->greaterThan($targetIn))
                        ...
                        @endphp
                        <div class="mt-3">
                            <span class="bg-red-50 text-red-600 border border-red-200 px-3 py-1.5 rounded-xl text-[10px] font-bold inline-flex items-center gap-1.5 shadow-sm">
                                <i class="fa-solid fa-triangle-exclamation animate-pulse"></i> Telat: {{ trim($lateText) }}
                            </span>
                        </div>
                    @else
                        <div class="mt-3">
                            <span class="bg-green-50 text-green-600 border border-green-200 px-3 py-1.5 rounded-xl text-[10px] font-bold inline-flex items-center gap-1.5 shadow-sm">
                ...
                @if($permit->hod_message)
                    <div class="mt-3 bg-gray-50 p-3 rounded-xl border border-gray-200 text-xs text-gray-600">
                ...
                <div class="flex items-center gap-3 mt-5 pt-4 border-t border-gray-100 flex-wrap">
                    {{-- QR & Print for approved/out --}}
                    @if(in_array($permit->status, ['approved', 'out']))
                        <button @click="qrModalOpen = true; activeUuid = '{{ $permit->uuid }}'; activeCode = '{{ $permit->unique_code }}'" 
                    ...
                    @if($permit->status == 'pending')
                        <a href="{{ route('employee.permit.edit', $permit->id) }}"
                           class="inline-flex items-center gap-2 px-5 py-2.5 bg-white border border-blue-200 text-blue-600 rounded-xl font-bold text-xs hover:bg-blue-50 hover:border-blue-300 transition-all shadow-sm">
                            <i class="fa-solid fa-pen-to-square"></i> Edit
                        </a>
                        <form action="{{ route('employee.permit.destroy', $permit->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Apakah Anda yakin ingin menghapus pengajuan ini secara permanen?');">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="inline-flex items-center gap-2 px-5 py-2.5 bg-white border border-red-200 text-red-600 rounded-xl font-bold text-xs hover:bg-red-50 hover:border-red-300 transition-all shadow-sm">
                                <i class="fa-solid fa-trash-can"></i> Hapus
                            </button>
                        </form>
                    @endif

                    {{-- No action for finished statuses --}}
                    @if(in_array($permit->status, ['cancelled', 'rejected', 'returned', 'expired']))
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-16 text-center">
        ...
        <h3 class="text-lg font-bold text-gray-600 mb-2">Belum Ada Pengajuan Izin</h3>
        <p class="text-sm text-gray-400 mb-6">Buat pengajuan izin baru untuk mendapatkan tiket dan QR Code.</p>
        <a href="{{ route('employee.permit.create') }}" class="inline-flex items-center gap-2 bg-gradient-to-r from-mna-dark to-mna-teal text-white px-6 py-3 rounded-xl font-bold text-sm shadow-lg shadow-mna-teal/20 hover:shadow-xl transition-all">
    ...
    <div x-show="qrModalOpen" style="display: none;" 
        ...
        <div class="relative w-80" @click.away="qrModalOpen = false">
            <div class="bg-white rounded-3xl p-8 shadow-2xl transform transition-all duration-500 hover:scale-105 border-t-4 border-mna-teal text-center relative overflow-hidden">
                ...
                <p class="text-[10px] text-mna-teal font-bold mb-4 relative z-10 bg-mna-light px-3 py-1 rounded-lg inline-block" x-text="activeCode"></p>
    ...
    <div x-show="cancelModalOpen" style="display: none;" 
        ...
        <div @click.away="cancelModalOpen = false" class="bg-white rounded-2xl w-full max-w-md p-6 shadow-2xl relative transform transition-all">
            ...
            <p class="text-sm text-gray-500 mb-6">Silakan isi alasan mengapa pengajuan izin ini dibatalkan.</p>
            
            <form :action="'{{ url('employee/permit') }}/' + cancelId + '/cancel'" method="POST">
                @csrf
                @method('PATCH')
                
                <div class="mb-6">
                    <label class="block text-sm font-bold text-gray-700 mb-2">Alasan Pembatalan <span class="text-red-500">*</span></label>
                    <textarea name="cancel_message" rows="3" required class="w-full rounded-xl border-gray-200 bg-gray-50 p-3 text-sm focus:bg-white focus:ring-2 focus:ring-red-500 outline-none transition" placeholder="Contoh: Keperluan pribadi sudah terselesaikan, dll..."></textarea>
                </div>
